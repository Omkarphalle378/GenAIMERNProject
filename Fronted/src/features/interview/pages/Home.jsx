...
const Home = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        if (!resumeFile && !selfDescription?.trim()) {
            alert("Please upload a PDF resume or enter a self description")
            return
        }
        try {
            ...
            if (data?._id) {
                ...
                alert("Could not generate the interview plan. Check the server and try again.")
            }
        } catch (err) {
                    ...
                    <ul className='reports-list'>
                        {reports.map((report, index) => (
                            <li key={report._id} className='report-item' onClick={() => navigate(`/interview/${report._id}`)}>
                                <h3>{report.title || `Interview Plan ${index + 1}`}</h3>
